...
(define (find-blank puz)
  (local [;; (get-guess puz) Consumes a puz and produces a list 
          ;;   of Guess strutures
          ;; get-guess: Puzzle -> (listof Guess)          
          (define (get-guess puz)
            (filter (λ (x) (symbol=? (guess-symbol x)
                                     (first (first (puzzle-constraints puz)))))
                    (foldr append empty (map (λ (y) (filter guess? y)) 
                                             (puzzle-board puz)))))
          
          ;; (get-symbol lst) Consumes a list of Guess structures , lst,
          ;;   and produces a list of the Guess' symbols
          ;; get-symbol: (listof Guess) -> (listof Sym)
          (define (get-symbol lst)
            (map (λ (x)(guess-symbol x)) lst))       
          
          ;; (find-blank/acc puz row col) accumulative part of find-blank
          ;;   with row and col as accumulators
          ;; find-blank/acc: Puzzle Nat Nat -> (anyof Pos false 'guess)
          (define (find-blank/acc puz row col)
            (cond [(or (empty? (puzzle-constraints puz))
                       (andmap (λ (x)(andmap number? x))(puzzle-board puz)))
                   false]
                  [(empty? (first (puzzle-board puz)))
                   (find-blank/acc 
                    ...
                    (add1 row) 0)]
                  [(and (symbol? (first (first (puzzle-board puz))))
                        (symbol=? (first (first (puzzle-board puz)))
                                  (first (first (puzzle-constraints puz)))))
                   (make-posn col row)]
                  [(and 
                    (not (ormap 
                          (λ (x)(member? 
                                 (first (first (puzzle-constraints puz))) x))
                          (puzzle-board puz)))
                    (andmap 
                     (λ (x)(symbol=? x (first (get-symbol (get-guess puz))))) 
                     (cons (first (first (puzzle-constraints puz)))
                           (get-symbol (get-guess puz))))) 'guess]
                  
                  [else (find-blank/acc (make-puzzle
                                         (puzzle-size puz)
                                         (cons (rest (first (puzzle-board puz)))
                                               (rest (puzzle-board puz)))
                                         (puzzle-constraints puz))
                                        row (add1 col))]))]
    (find-blank/acc puz 0 0)))
...
(define (used-in-column puz pos)
  (quicksort (map (λ (x) (cond [(guess? x)(guess-number x)]
                               [(number? x) x])) 
                  (filter (λ (y) (not (symbol? y)))
                          (map (λ (x)(list-ref x (posn-x pos)))
                               (puzzle-board puz)))) <))
...
(define (neighbours puz)
  (local [(define pos (find-blank puz))]
    (cond [(false? pos) empty]
          [(guess-valid? puz)
           (list (apply-guess puz))]
          [else      
           (map (λ (x) (fill-in-guess puz pos x))
                (available-vals puz pos))])))
